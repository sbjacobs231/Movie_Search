$(function() {
	...
	$(document).on("click", "#submit", function(event) {
		...
		function showMovies(data) {
			...
			if (data.Response === "False") {
				...
				movieHTML += "<i class='material-icons icon-help'>help_outline</i>No movies found that match: " + $('#search').val();
				movieHTML += "</li>";
			} else {
				$.each(data.Search, function(i, movie) {
					...
					if (movie.Poster == "N/A") {
						movieHTML += '<i class="material-icons poster-placeholder">crop_original</i></a></div>';
					} else {
						movieHTML += '<img class="movie-poster" src="' + movie.Poster + '"></a></div>';
					}
	...
	$(document).on("click", '.details', function(event) {
		movieID = $(this).attr('class').substring(8);
		var movieDetails = {
			i: movieID
		}
		function showMovie(movie) {
			...
			if (movie.Poster == "N/A") {
				htmlText += "<div class='about'><i class='material-icons poster-placeholder'>crop_original</i>";
			} else {
			...
			htmlText += "<a href='" + link + "' target='_blank' id='viewOnIMDB'>View on IMDB</a></div>";
			// Movie Details Div
